...
@section("content")
    <section>
    <form id="form_blog" action="{{ route('admin.blog.update', $data->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="d-flex flex-wrap justify-content-between align-items-center mb-3">
                <div>
                    <h4 class="mb-1 mt-3">Chỉnh sửa bài viết</h4>
                    <p class="text-muted">Bài viết: {{$data->title}}</p>
                </div>
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="outputSlug" placeholder="" name="slug" value="{{ old('slug', $data->slug) }}" />
                            </div>
                            <div class="mb-3">
                ...
                <div class="col-12 col-lg-4">
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            ...
                            <div class="fallback">
                                <input id="thumbnail" name="thumbnail" type="hidden" value="{{ old('thumbnail', $data->thumbnail) }}" required />
                            </div>
                        </div>
                    </div>
                    <div class="card mb-4">
                            ...
                            <div class="mb-3">
                                ...
                                <select id="status" class="select2 form-select" name="status">
                                    @foreach ($statusLabels as $status => $label)
                                        <option value="{{ $status }}" @selected(old('status', $data->status) == $status)>{{ $label }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="mb-3 select2-danger">
                                ...
                                <select id="hashtag" class="select2 form-select" multiple data-allow-clear="true"
                                    ...
                                    @foreach ($hashtags as $item)
                                        <option value="{{ $item->id }}" @selected(in_array($item->id, old('hashtags', ($data->hashtags ? explode(', ', $data->hashtags) : []))))>{{ $item->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="is_comment" name="is_comment" @checked(old('is_comment', $data->is_comment)) />
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="meta_title" name="meta_title"
                                    value="{{ old('meta_title', $data->meta_title) }}" placeholder="Nhập meta title..." />
                            </div>
                            <div class="mb-3">
                                <label class="form-label" for="meta_description">Meta Description <span
                                        class="text-muted">(Optional)</span></label>
                                <textarea class="form-control" id="meta_description" name="meta_description" rows="2"
                                    placeholder="Nhập meta description...">{{ old('meta_description', $data->meta_description) }}</textarea>
                            </div>
                            <div class="mb-3">
